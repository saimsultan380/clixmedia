      ...
      <div className="max-w-7xl mx-auto flex flex-col items-center text-center relative z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 w-full">
          {deliverables.map((item, i) => (
            <div
              key={i}
              className="solution-fade group relative p-8 bg-[#0A0B0D] rounded-3xl border border-white/5 transition-all duration-500 hover:-translate-y-2 hover:border-white/20 hover:shadow-[0_10px_40px_rgba(255,59,0,0.1)] text-left flex flex-col justify-between min-h-[160px]"
            >
              {/* Card Corner Glow */}
              <div className="absolute -top-12 -right-12 w-32 h-32 bg-accent/20 rounded-full blur-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />

              {/* --- CONNECTIVE LINES --- */}
              {/* Horizontal line inside card (Tablet/Desktop) */}
              <div className="hidden sm:block absolute top-[47px] left-[64px] right-0 h-[1px] bg-white/10 z-0" />
              {/* Horizontal gap line (Desktop - 4 cols) */}
              {i % 4 !== 3 && (
                <div className="hidden lg:block absolute top-[47px] -right-[16px] w-[16px] h-[1px] bg-white/10 z-0" />
              )}
              {/* Horizontal gap line (Tablet - 2 cols) */}
              {i % 2 !== 1 && (
                <div className="hidden sm:block lg:hidden absolute top-[47px] -right-[16px] w-[16px] h-[1px] bg-white/10 z-0" />
              )}

              {/* Vertical line inside card (Mobile) */}
              <div className="sm:hidden absolute top-[64px] left-[47px] bottom-0 w-[1px] bg-white/10 z-0" />
              {/* Vertical gap line (Mobile) */}
