...
export function ProcessTimeline() {
  ...
  const StepContent = ({ step, i, align }: { step: any, i: number, align: 'left' | 'right' }) => (
    <div className={`relative flex flex-col ${align === 'right' ? 'md:items-end md:text-right' : 'md:items-start md:text-left'} pl-12 md:pl-0 z-10 w-full`}>
      {/* Mobile Dot */}
      <div className="md:hidden absolute left-[4px] top-2 w-6 h-6 rounded-full bg-black border border-accent flex items-center justify-center z-20">
        <div className="w-2 h-2 rounded-full bg-accent" />
      </div>

      {/* Mobile Bottom Mask (hides line below last dot) */}
      {i === steps.length - 1 && (
        <div className="md:hidden absolute left-[0px] top-[20px] bottom-[-200px] w-[30px] bg-black z-10" />
      )}
      ...
      <span className="w-fit text-accent font-mono text-sm tracking-widest uppercase mb-4 inline-block bg-transparent px-3 py-1 rounded-full border border-white/20 relative z-20">Phase 0{i + 1}</span>
    ...
    <section ref={containerRef} className="px-6 md:px-12 py-32 bg-black relative overflow-hidden">
      <div className="max-w-5xl mx-auto">
        <div className="flex justify-center mb-24">
          <div className="inline-flex items-center space-x-2 border border-white/20 rounded-full px-6 py-2 text-sm font-mono tracking-widest text-content-muted bg-transparent">
        ...
        <div className="relative">
          ...
          <div className="progress-line-desktop hidden md:block absolute left-1/2 top-0 w-[2px] -translate-x-1/2 bg-accent h-0 rounded-full z-0" />

          {/* Mobile Left Line */}
          <div className="md:hidden absolute left-[15px] top-[20px] bottom-0 w-[2px] bg-white/5 rounded-full z-0" />
          <div className="progress-line-mobile md:hidden absolute left-[15px] top-[20px] w-[2px] bg-accent h-0 rounded-full z-0" />
          ...
          <div className="flex flex-col space-y-12 md:space-y-24 relative z-20">
            {steps.map((step, i) => (
              <div key={i} className="step-item relative flex flex-col md:flex-row items-center justify-between w-full group">
                ...
                <div className="hidden md:flex relative z-20 w-10 h-10 rounded-full border-2 border-white/10 bg-black items-center justify-center group-hover:border-accent transition-all duration-500">
